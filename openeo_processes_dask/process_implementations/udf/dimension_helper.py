...
def fix_udf_dimensions(
    cube: xr.DataArray, context: Optional[dict[str, Any]] = None
) -> xr.DataArray:
    """
...
def assign_dimension_labels(
    ...
    try:
        ...
        warnings.warn(
            f"Coordinate assignment failed: {e}. Returning cube without coordinates."
        )
        return cube
...
def restore_semantic_dimensions(
    ...
    try:
        ...
        warnings.warn(
            f"Semantic dimension restoration failed: {e}. Returning original cube."
        )
        return cube
